...
from execution_testing.cli.pytest_commands.plugins.filler.verify_traces import (
...
def _write_trace_file(
    ...
    with path.open("w") as f:
        ...
        f.write(
            json.dumps({"output": output, "gasUsed": gas_used})
            + "\n"
        )
...
class TestLoadTracesFromDumpDir:
    ...
    def test_single_call_dir_two_trace_files(self, tmp_path: Path):
        """Single call dir with two trace files returns one Traces."""
        call_dir = tmp_path / "0"
        call_dir.mkdir()
        _write_trace_file(
            call_dir / "trace-0-0xaaa.jsonl"
        )
        _write_trace_file(
            call_dir / "trace-1-0xbbb.jsonl"
        )
    ...
    def test_multiple_call_dirs(self, tmp_path: Path):
        """Multiple call dirs (0, 1, 2) return correctly ordered list."""
        for i in range(3):
            call_dir = tmp_path / str(i)
            call_dir.mkdir()
            _write_trace_file(
                call_dir / f"trace-0-0x{i:03x}.jsonl"
            )
    ...
    def test_non_numeric_subdirs_ignored(self, tmp_path: Path):
        """Non-numeric subdirectories are ignored."""
        (tmp_path / "0").mkdir()
        _write_trace_file(
            tmp_path / "0" / "trace-0-0xaaa.jsonl"
        )
    ...
    def test_numeric_sorting_not_lexical(self, tmp_path: Path):
