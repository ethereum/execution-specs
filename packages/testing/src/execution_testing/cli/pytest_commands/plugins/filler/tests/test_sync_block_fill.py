...
MARKED_MODULE = textwrap.dedent(
    ...
    def single_block_test(blockchain_test, pre, value):
        tx = Transaction(
    ...
    def test_marked_state_context(blockchain_test, pre) -> None:
        single_block_test(blockchain_test, pre, 1)


    @pytest.mark.no_sync_block_timestamp_headroom
    def test_marked_timestamp_headroom(blockchain_test, pre) -> None:
        single_block_test(blockchain_test, pre, 2)


    def test_unmarked(blockchain_test, pre) -> None:
        single_block_test(blockchain_test, pre, 3)
    """
)
...
def test_marked_tests_fill_without_the_block(
    pytester: pytest.Pytester,
) -> None:
    """
    A test marked unable to carry the extra block fills as exactly the
    author's chain rather than being skipped, and its neighbours keep
    their appended block.
    """
    test_module = make_test_module(
        pytester, MARKED_MODULE, "test_eligibility.py"
    )
    output = fill(pytester, test_module)

    fixtures = fixtures_of_format(output, "blockchain_tests_engine_x")
    assert len(fixtures) == 3, "marked tests must still be filled"
    for test_id, fixture in fixtures.items():
        assert len(fixture["engineNewPayloads"]) == 1
        if "::test_marked_" in test_id:
            assert "syncPayload" not in fixture
        else:
            assert_appended_above(fixture, blocks=1)
