"""Health check tests for validate client binaries."""
...
CONFIG_FILES = ("validate.toml", "consume-direct.toml")

SANITY_DIR = Path(__file__).parent.parent / "sanity_fixtures"
...
def load_config() -> Dict[str, Dict[str, str]]:
    """Load client configuration from validate.toml or consume-direct.toml."""
    config_file = _find_config_file()
    config_path = Path.cwd() / config_file
    if not config_path.exists():
        pytest.skip(
            f"No config file found ({', '.join(CONFIG_FILES)}). "
            f"Create one to configure client binaries."
        )
...
def get_consumer(client: str, bin_path: Path) -> Any:
    ...
    block_bin_str = client_config.get("block-bin", "")
    if block_bin_str:
        kwargs["block_binary"] = Path(block_bin_str).expanduser().resolve()

    engine_bin_str = client_config.get("engine-bin", "")
    if engine_bin_str:
        kwargs["engine_binary"] = Path(engine_bin_str).expanduser().resolve()
...
def get_bin_path(client: str, test_type: str | None = None) -> Path:
    """Resolve binary path for a client from config.

    Support per-type binary overrides (e.g. state-bin for reth/revm).
...
def run_health(client: str, test_type: str) -> None:
    ...
    assert fixture_file is not None, (
        f"sanity fixture missing for {test_type}"
    )
...
def test_nimbus_block() -> None:
    """Nimbus block test sanity check."""
    run_health("nimbus", "block")


def test_nimbus_engine() -> None:
    """Nimbus engine test sanity check."""
    run_health("nimbus", "engine")
