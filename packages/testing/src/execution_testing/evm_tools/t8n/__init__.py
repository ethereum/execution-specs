...
def transition_fork_criteria(fork: "TransitionFork") -> ByTimestamp:
    """
    Return the spec criteria at which `fork` activates its target fork.

    Only timestamp-scheduled transitions are supported. Block-number
    transitions exist only as legacy CLI aliases, which `find_fork`
    resolves.
    """
    if not fork.at_timestamp:
        raise ValueError(
            f"{fork.name()} is scheduled by block number; only timestamp "
            "transitions derive fork-block activation from the schedule"
        )
