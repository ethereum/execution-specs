...
def sync_block_context_unavailable(
    ...
    # takes its slot number plus one. Nothing else on the fill side
    # notices every overflow - Python integers do not wrap and `t8n`
    # accepts some of the values - so an unguarded fill can emit a payload
    # no client can parse. The fields are semantic and never clamped or
    # shifted; the fill declines the block instead.
