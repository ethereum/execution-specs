...
At the fork block, set the nonce of the 28 Mainnet accounts that have
empty code, a zero nonce, and non-empty storage to one, so that they fail
the EIP-684 contract creation precondition without a runtime storage
check.

https://eips.ethereum.org/EIPS/eip-8253
"""
...
ZERO_NONCE_STORAGE_ACCOUNTS: List[Address] = [
    Address(address, label="ZERO_NONCE_STORAGE_ACCOUNT")
    for address in [
...
"""
The Mainnet accounts whose nonce is bumped to one at the fork block, in the
order of the `targeted-accounts.json` asset of the EIP (by address hash).
"""


class EIP8253(BaseFork):
    """EIP-8253 class."""
