...
class EIP8253(BaseFork):
    """EIP-8253 fork metadata and fixture pre-allocation."""

    @classmethod
    def eip_8253_targeted_accounts(cls) -> tuple[Address, ...]:
        """Return the fixed Mainnet account list."""
        return tuple(
            Address(address) for address in EIP_8253_TARGETED_ACCOUNTS
        )

    @classmethod
    def pre_allocation(cls) -> Mapping:
        """Represent the already-transitioned state in state tests."""
        return {
            address: {"nonce": 1, "balance": 1, "storage": {0: 1}}
            for address in EIP_8253_TARGETED_ACCOUNTS
        } | super(EIP8253, cls).pre_allocation()  # type: ignore

    @classmethod
    def pre_allocation_blockchain(cls) -> Mapping:
        """Seed the pre-transition account shape in blockchain tests."""
        return {
            address: {"nonce": 0, "balance": 1, "storage": {0: 1}}
            for address in EIP_8253_TARGETED_ACCOUNTS
        } | super(EIP8253, cls).pre_allocation_blockchain()  # type: ignore
