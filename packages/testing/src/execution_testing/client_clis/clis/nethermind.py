...
class Nethtest(EthereumCLI):
    ...
    def __init__(
        ...
        self.needs_dotnet = self.detect_dotnet(binary)

    @staticmethod
    def detect_dotnet(binary: Path) -> bool:
    ...
    def build_base_command(self, args: List[str]) -> List[str]:
        """Build a command, wrapping with dotnet if needed."""
        if self.needs_dotnet:
    ...
    def help(self, subcommand: str | None = None) -> str:
        ...
        return self.run_command(help_command).stdout
...
class NethtestFixtureConsumer(
    ...
    @property
    def exception_mapper_prop(self) -> ExceptionMapper:
    ...
    def get_dir_results(
        ...
        if cache_key not in self.dir_cache:
            ...
            if debug_output_path:
                extra_args += ["--trace"]
            command = self.build_base_command(extra_args)

            result = self.run_command(command)

            if debug_output_path:
                self.validate_debug_dump(command, result, debug_output_path)
    ...
    def validate_test(
        ...
        if fixture_name:
            ...
            validate_test_result(
                self.fixture_cache, self.exception_mapper_prop,
    ...
    def consume_fixture(
        ...
        self.validate_test(fixture_format, label, fixture_path, fixture_name, debug_output_path)
