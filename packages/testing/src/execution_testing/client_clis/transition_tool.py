...
class TransitionTool(EthereumCLI):
    ...
    class TransitionToolData:
        ...
        def active_fork(self) -> Fork:
            """
            Return the fork whose rules apply to this block.

            `fork` may be a transition fork; the block's number and
            timestamp pick the side of the boundary it falls on.
            """
        ...
        def fork_name(self) -> str:
            """
            Return the name of the block's fork for external tools.

            External tools always get the resolved fork, never the
            transition name. The in-process EELS t8n reads `fork` itself
            and derives fork-block activation from the transition schedule.
            """
