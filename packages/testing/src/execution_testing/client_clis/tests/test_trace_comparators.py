...
def _make_trace_line(**overrides) -> TraceLine:
...
class TestTransactionCountMismatch:
    ...
    def test_is_trace_difference_subclass(self):
        """Verify it is a TraceDifference subclass."""
        mismatch = TransactionCountMismatch(
            baseline_count=3, current_count=2
        )
...
class TestTraceComparisonResult:
    ...
    def test_equivalent_when_no_differences(self):
    ...
    def test_not_equivalent_when_differences_exist(self):
        ...
        result = TraceComparisonResult(
            equivalent=False, differences=[diff]
        )
...
class TestTraceComparatorABC:
    ...
    def test_mismatched_transaction_count(self, spy_comparator):
    ...
    def test_empty_traces_are_equivalent(self, spy_comparator):
    ...
    def test_single_transaction_delegates_once(self, spy_comparator):
...
class TestCreateComparator:
    ...
    def test_create_supported_comparators(
        self, comparator_type, expected_name
    ):
...
class TestTraceLineCompare:
    ...
    def test_gas_excluded_by_default(self):
    ...
    def test_custom_exclude_fields(self):
        ...
        b_diff, c_diff = baseline.compare(
            current, exclude_fields={"pc"}
        )
        assert "pc" not in b_diff
        assert "gas" in b_diff

    def test_are_equivalent_with_custom_exclude(self):
...
class TestTransactionTracesCompare:
    ...
    def test_identical_traces_empty_diffs(self):
    ...
    def test_different_gas_used_without_post_processing(self):
        ...
        diffs = baseline.compare(
            current, enable_post_processing=False
        )
        assert any(
            d.line_index is None
            and "gas_used" in d.baseline_fields
            for d in diffs
        )

    def test_different_gas_used_with_post_processing(self):
        ...
        current.gas_used = HexNumber(0x6000)
        diffs = baseline.compare(
            current, enable_post_processing=True
        )
        assert not any(
            d.line_index is None
            and "gas_used" in d.baseline_fields
    ...
    def test_single_field_diff_on_line(self):
        """Single-field difference on a line returns line index and fields."""
        baseline = _make_transaction_traces(
            [_make_trace_line(pc=0)]
        )
        current = _make_transaction_traces(
            [_make_trace_line(pc=5)]
        )
        diffs = baseline.compare(current)
        assert len(diffs) == 1
    ...
    def test_multiple_fields_diff_on_one_line(self):
        """Multiple field diffs on one line are grouped together."""
        baseline = _make_transaction_traces(
            [_make_trace_line(pc=0, op=0x60)]
        )
        current = _make_transaction_traces(
            [_make_trace_line(pc=5, op=0x61)]
        )
        diffs = baseline.compare(current)
        assert len(diffs) == 1
        assert "pc" in diffs[0].baseline_fields
    ...
    def test_exclude_fields(self):
        ...
        current = _make_transaction_traces(
            [_make_trace_line(gas=0x200, pc=5)]
        )
        diffs = baseline.compare(
            current, exclude_fields={"gas", "gas_cost"}
        )
        assert len(diffs) == 1
        assert "pc" in diffs[0].baseline_fields
        assert "gas" not in diffs[0].baseline_fields

    def test_exclude_fields_all_diffs_excluded(self):
    ...
    def test_post_processing_removes_gas_stack_pollution(self):
        ...
        next_line_baseline = _make_trace_line(
            pc=1, op=0x60, op_name="PUSH1", depth=1,
            stack=[0xAAAA],
        )
        ...
        baseline = _make_transaction_traces(
            [gas_line, next_line_baseline]
        )
        current = _make_transaction_traces(
            [gas_line, next_line_current]
        )
...
class TestTransactionTracesAreEquivalentRegression:
    ...
    def test_gas_only_difference_is_equivalent(self):
        ...
        assert baseline.are_equivalent(
            current, enable_post_processing=False
        )

    def test_pc_difference_not_equivalent(self):
    ...
    def test_gas_used_checked_without_post_processing(self):
    ...
    def test_gas_used_ignored_with_post_processing(self):
...
class TestExactTraceComparator:
    ...
    def test_identical_traces_are_equivalent(self, comparator):
    ...
    def test_single_field_difference(
        self, comparator, field, baseline_val, current_val
    ):
    ...
    def test_differing_stack(self, comparator):
    ...
    def test_different_trace_lengths(self, comparator):
    ...
    def test_different_output(self, comparator):
    ...
    def test_multiple_differences_in_one_transaction(self, comparator):
    ...
    def test_assembly_format_baseline_and_current(self, comparator):
    ...
    def test_full_compare_traces_multi_transaction(self, comparator):
        """Integration: multi-tx Traces with one differing tx."""
        identical_tx = _make_transaction_traces(
            [_make_trace_line(pc=0)]
        )
        baseline_diff_tx = _make_transaction_traces(
            [_make_trace_line(pc=10)]
        )
        current_diff_tx = _make_transaction_traces(
            [_make_trace_line(pc=20)]
        )
        ...
        assert result.equivalent is False
        assert all(
            d.transaction_index == 1 for d in result.differences
        )
...
class TestExactNoGasTraceComparator:
    ...
    def test_gas_and_non_gas_difference(self, comparator):
    ...
    def test_stack_difference_detected(self, comparator):
        """Non-gas fields like stack are still checked."""
        baseline = _make_transaction_traces(
            [_make_trace_line(stack=[0x1])]
        )
        current = _make_transaction_traces(
            [_make_trace_line(stack=[0x2])]
        )
        result = comparator.compare_transaction_traces(
            baseline, current, 0
        )
    ...
    def test_length_mismatch(self, comparator):
    ...
    def test_output_mismatch(self, comparator):
        """Different output field is detected."""
        baseline = _make_transaction_traces(output="0xaa")
        current = _make_transaction_traces(output="0xbb")
        result = comparator.compare_transaction_traces(
    ...
    def test_gas_used_difference_is_equivalent(self, comparator):
    ...
    def test_gas_stack_pollution_is_equivalent(self, comparator):
        ...
        baseline = _make_transaction_traces(
            [gas_line, next_baseline]
        )
        current = _make_transaction_traces(
            [gas_line, next_current]
        )
        result = comparator.compare_transaction_traces(
            baseline, current, 0
        )
