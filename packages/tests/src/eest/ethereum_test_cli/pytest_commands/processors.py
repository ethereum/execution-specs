    ...
    def process_args(self, args: List[str]) -> List[str]:
        ...
        if self.command_name == "engine":
            modified_args.extend(
                [
                    "-p",
                    "eest.ethereum_test_cli.pytest_commands.plugins.consume.simulators.engine.conftest",
                ]
            )
            ...
            modified_args.extend(
                [
                    "-p",
                    "eest.ethereum_test_cli.pytest_commands.plugins.consume.simulators.sync.conftest",
                ]
            )
            ...
            modified_args.extend(
                [
                    "-p",
                    "eest.ethereum_test_cli.pytest_commands.plugins.consume.simulators.rlp.conftest",
                ]
            )
        else:
