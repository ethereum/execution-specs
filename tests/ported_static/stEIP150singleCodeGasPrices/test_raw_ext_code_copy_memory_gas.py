...
`Op.GAS`. EIP-8038 raises the cold account access
...
the `COLD_ACCOUNT_ACCESS` rise plus `WARM_ACCESS`. The cold term is
the fork's own constant less Cancun's. The extra warm term is gated on
the `is_eip_enabled(8037)` flag (the registered flag that activates
the repricing at Amsterdam), so the delta is exactly 0 on earlier
