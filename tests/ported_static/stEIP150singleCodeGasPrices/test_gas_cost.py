...
def test_gas_cost(
    ...
    expect_posts: list[dict] = [
        ...
        {
            addr: Account(
                storage=_storage_with_any({0: 700 + cold_storage_delta}, [1])
            )
        },
        {addr: Account(storage=_storage_with_any({0: 1500}, [1]))},
        {
            addr: Account(
                storage=_storage_with_any({0: 2000 + cold_account_delta}, [1])
            )
        },
        {
            addr: Account(
                storage=_storage_with_any({0: 1300 + cold_account_delta}, [1])
            )
        },
        {
            addr: Account(
