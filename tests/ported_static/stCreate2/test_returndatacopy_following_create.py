...
def test_returndatacopy_following_create(
    ...
    # Source: lll
    # { (CREATE2 0 0 (lll (seq (MSTORE 0 0x0000111122223333444455556666777788889999aaaabbbbccccddddeeeeffff) (RETURN 0 32)) 0) 0) (RETURNDATACOPY 0 0 32) (SSTORE 0 (MLOAD 0)) }
    ...
    # Source: lll
    # { (seq (create2 0 0 (lll (STOP) 0) 0) (RETURNDATACOPY 0 0 32) (SSTORE 0 (MLOAD 0)) )}
    contract_2 = pre.deploy_contract(
        code=Op.PUSH1[0x0] + Op.PUSH1[0x2]
        + Op.CODECOPY(dest_offset=0x0, offset=0x1f, size=Op.DUP1)
        + Op.PUSH1[0x0] * 2 + Op.POP(Op.CREATE2)
        + Op.RETURNDATACOPY(dest_offset=0x0, offset=0x0, size=0x20)
        + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x0)) + Op.STOP + Op.INVALID
