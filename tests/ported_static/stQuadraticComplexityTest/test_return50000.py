...
def test_return50000(
    ...
    addr_0xaaaf5374fce5edbc8e2a8697c15331677e6ebf0b = pre.deploy_contract(
        code=Op.RETURN(offset=Op.CALLDATALOAD(offset=0xc34f), size=0x1) + Op.STOP,
        balance=0xfffffffffffff,
        nonce=0,
        address=Address("0xaab87d565dd96e58089e1dff410fbdac45290658"),  # noqa: E501
    )
    # Source: lll
    # { (def 'i 0x80) (for {} (< @i 50000) [i](+ @i 1) [[ 0 ]] (CALL 1564 <contract:0xaaaf5374fce5edbc8e2a8697c15331677e6ebf0b> 0 0 50000 0 0) ) [[ 1 ]] @i }
    target = pre.deploy_contract(
        code=Op.JUMPDEST
        + Op.JUMPI(pc=0x3f, condition=Op.ISZERO(Op.LT(Op.MLOAD(offset=0x80), 0xc350)))
        + Op.SSTORE(key=0x0, value=Op.CALL(gas=0x61c, address=0xaab87d565dd96e58089e1dff410fbdac45290658, value=0x0, args_offset=0x0, args_size=0xc350, ret_offset=0x0, ret_size=0x0))  # noqa: E501
    ...
    expect_entries_: list[dict] = [
        ...
        target: Account(
                storage={},
                code=bytes.fromhex("5b61c3506080511015603f576000600061c3506000600073aab87d565dd96e58089e1dff410fbdac4529065861061cf16000556001608051016080526000565b60805160015500"),  # noqa: E501
                nonce=0,
            ),
    },
        },
