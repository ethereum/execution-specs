...
def test_static_ab_acalls2(
    ...
    addr_0x945304eb96065b2a98b57a48a06ae28d285a71b5 = pre.deploy_contract(
        code=Op.SSTORE(key=0x0, value=Op.ADD(Op.SLOAD(key=0x0), 0x1))
        + Op.STATICCALL(gas=Op.SUB(Op.GAS, 0x186a0), address=0xcee890df61958e0d40fbbfc310af80b8c47d0dfe, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)
        + Op.STOP,
        nonce=0,
        address=Address("0xe278f8058bef1396c2b1df4d1dc4b65233133c57"),  # noqa: E501
    )
    # Source: lll
    # {  ( MSTORE 0 (ADD (MLOAD 0) 1)) (STATICCALL (- (GAS) 100000) <contract:0x245304eb96065b2a98b57a48a06ae28d285a71b5> 0 0 0 0) }
    addr_0x195e7baea6a6c7c4c2dfeb977efac326af552d87 = pre.deploy_contract(
        ...
        + Op.STATICCALL(gas=Op.SUB(Op.GAS, 0x186a0), address=0x57efc7a25d8e40b0798fb4cbc2bcc3c124141cbb, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)
        + Op.STOP,
        balance=0xde0b6b3a7640000,
        nonce=0,
    ...
    # Source: lll
    # { ( MSTORE 0 (ADD (MLOAD 0) 1)) (STATICCALL (- (GAS) 100000) <contract:0x195e7baea6a6c7c4c2dfeb977efac326af552d87> 0 0 0 0) }
    addr_0x245304eb96065b2a98b57a48a06ae28d285a71b5 = pre.deploy_contract(
        code=Op.MSTORE(offset=0x0, value=Op.ADD(Op.MLOAD(offset=0x0), 0x1))
        + Op.STATICCALL(gas=Op.SUB(Op.GAS, 0x186a0), address=0xdb486d3e181181d2063032b1250c07ca0185a446, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)
        + Op.STOP,
        nonce=0,
