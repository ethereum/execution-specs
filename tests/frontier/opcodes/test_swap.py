...
def test_swap(
    ...
    # `contract_code.gas_cost(fork)` covers all PUSHes, the SWAP, and the
    # 16 SSTOREs (regular + state under EIP-8037). Some SSTOREs write zero,
    # which the default cold zero->non-zero assumption over-estimates;
    # harmless. EIP-1706 slack on the trailing SSTORE.
    tx = Transaction(
        sender=pre.fund_eoa(),
        to=contract_address,
        gas_limit=(
