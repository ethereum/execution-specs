...
from ethereum.exceptions import NonceTooHighError
...
def test_high_nonce(test_file_high_nonce: str) -> None:
    test = load_istanbul_transaction(test_dir, test_file_high_nonce)

    tx = rlp.decode_to(Transaction, test["tx_rlp"])

    with pytest.raises(NonceTooHighError):
...
def test_nonce(test_file_nonce: str) -> None:
    test = load_istanbul_transaction(test_dir, test_file_nonce)
