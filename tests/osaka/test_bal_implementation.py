...
class TestBALCore:
    ...
    def test_bal_builder_build_complete(self):
        ...
        add_storage_write(builder, address1, slot1, BlockAccessIndex(1), Bytes32(b'\x05' * 32))
        add_storage_read(builder, address1, slot2)
        add_balance_change(builder, address1, BlockAccessIndex(1), Bytes(b'\x00' * 16))
...
class TestBALTracker:
    ...
    def test_tracker_storage_write_actual_change(self, mock_capture):
        """Test tracking storage write with actual change."""
        from ethereum.osaka.block_access_lists.tracker import track_storage_write
        builder = BlockAccessListBuilder()
        tracker = StateChangeTracker(builder)
    ...
    @patch('ethereum.osaka.block_access_lists.tracker.capture_pre_state')
    def test_tracker_storage_write_no_change(self, mock_capture):
    ...
    def test_tracker_balance_change(self):
        """Test tracking balance changes."""
        from ethereum.osaka.block_access_lists.tracker import track_balance_change
        builder = BlockAccessListBuilder()
        tracker = StateChangeTracker(builder)
        ...
        track_balance_change(tracker, address, new_balance, mock_state)
    ...
    def test_tracker_nonce_change(self):
        ...
        from ethereum.osaka.block_access_lists.tracker import track_nonce_change
        builder = BlockAccessListBuilder()
        tracker = StateChangeTracker(builder)
        ...
        track_nonce_change(tracker, address, new_nonce, mock_state)
    ...
    def test_tracker_code_change(self):
        ...
        track_code_change(tracker, address, new_code, mock_state)
...
class TestBALIntegration:
    ...
    def test_system_contract_indices(self):
    ...
    def test_post_execution_index(self):
    ...
    def test_mixed_indices_ordering(self):
        """Test that mixed indices are properly ordered in the BAL."""
        builder = BlockAccessListBuilder()
        address = Bytes20(b'\x01' * 20)
        
        # Add changes with different indices (out of order)
        add_balance_change(builder, address, BlockAccessIndex(3), Bytes(b'\x03' * 16))
        add_balance_change(builder, address, BlockAccessIndex(1), Bytes(b'\x01' * 16))
        ...
        for i in range(4):
            ...
            assert account.balance_changes[i].post_balance == bytes([i]) * 16
...
class TestRLPEncoding:
    ...
    def test_rlp_encode_simple_bal(self):
        ...
        from ethereum.osaka.block_access_lists import rlp_encode_block_access_list
    ...
    def test_rlp_encode_complex_bal(self):
        ...
        add_storage_write(builder, address, slot, BlockAccessIndex(0), Bytes32(b'\x03' * 32))
...
class TestEdgeCases:
    ...
    def test_multiple_changes_same_slot(self):
        ...
        add_storage_write(builder, address, slot, BlockAccessIndex(0), Bytes32(b'\x00' * 32))
    ...
    def test_max_code_changes_constant(self):
