"""End-to-end: commit a block of spamoor storagespam transactions."""
...
from tests.benchmark.spamoor.helpers import build_storagespam_transactions


@pytest.mark.spamoor
@pytest.mark.testing_build_block
def test_storagespam_committed(
    ...
    """Deploy the storagespam stub and commit setRandomForGas calls."""
    raw_txs = build_storagespam_transactions(
        count=spamoor_config["count"],
        gas_units_to_burn=spamoor_config["gas_units_to_burn"],
        reuse_contract=spamoor_config["reuse_contract"],
