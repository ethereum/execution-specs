...
request matrices cannot express: the fee getter, value sent without
calldata, the fee decaying once demand stops, requests made while the
inhibitor is set, from an account whose code is delegated, from initcode,
with more value than the request costs, and through call types other than
CALL. The tests run against all such predeploys of the fork, so from
Amsterdam they cover the withdrawal and consolidation contracts as well as
the builder ones.
"""
...
pytestmark = [
    ...
    # The tests pin the predeploys' exact balances and excess, which earlier
    # transactions on a live network have already moved.
    pytest.mark.execute(
...
def test_fee_decays_to_minimum(
    ...
    target, whatever it dequeues; the access list pins every step down to
    zero, and a final request paying the minimum fee confirms the queue is
    open again.
    """
    predeploy = request_class.system_contract_address
