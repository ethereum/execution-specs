...
def test_access_list_floor_with_exact_balance(
    ...
    state_test(
        ...
        post=post,
        tx=tx,
        # Reach transaction validation instead of rejecting a nonempty block
        # whose gas used is zero because the invalid transaction was skipped.
        blockchain_test_rlp_modifier=(
            Header(gas_used=gas_limit) if gas_delta < 0 else None
        ),
    )
