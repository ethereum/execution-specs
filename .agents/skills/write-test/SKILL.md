...
- Build the input so it breaks only the rule under test, and expect the one exception the spec names. A client whose error maps to the wrong exception needs a mapper fix, not `exception=[A, B]`. Use a list only when a second violation cannot be avoided and the spec leaves the check order open, with a comment saying why each exception is valid.
