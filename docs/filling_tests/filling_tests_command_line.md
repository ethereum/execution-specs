...
## Filtering Fixture Formats

The `--formats` flag can be used to generate specific fixture output formats instead of all formats that a test naturally supports.

```console
uv run fill --formats=state_test tests/shanghai/
```

Multiple formats can be generated by providing a comma-separated list:

```console
uv run fill --formats=blockchain_test,blockchain_test_engine tests/shanghai/
```
