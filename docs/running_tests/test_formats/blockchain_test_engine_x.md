...
4. Verify the final state:
   - If [`postStateDiff`](#-poststatediff-optionalalloc) is present, apply the state differences to the pre-allocation group and verify that the result matches the client's final state.
   - If a `post` field is present, verify it directly.
