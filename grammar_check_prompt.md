...
You will create a PR to the `fix-grammar-tests` branch of `danceratopz/execution-specs`. Your task is to process ONE unchecked item from the grammar checklist.

## Step 1: Setup

```bash
git fetch origin fix-grammar-tests && git checkout fix-grammar-tests && git pull origin fix-grammar
```
...
Your branch must be based on fix-grammar-tests (not main). Rebase if needed:

```bash
git rebase fix-grammar-tests
```

**Verify**: Run `ls grammar_check_prompt.md` - if this file doesn't exist, run `git rebase fix-grammar-tests`.
...
Rebase on latest fix-grammar-tests and push:

```bash
git fetch origin fix-grammar-tests && git rebase origin/fix-grammar
git push -u origin HEAD
```

Provide the PR creation link targeting `fix-grammar-tests` (as clickable link, NOT in a code block):
