...
You will create a PR to the `fix-grammar-testing` branch of `danceratopz/execution-specs`. Your task is to process ONE unchecked item from the grammar checklist.

## Step 1: Setup

```bash
git fetch origin fix-grammar-testing && git checkout fix-grammar-testing && git pull origin fix-grammar
```
...
Your branch must be based on fix-grammar-testing (not main). Rebase if needed:

```bash
git rebase fix-grammar-testing
```

**Verify**: Run `ls grammar_check_prompt.md` - if this file doesn't exist, run `git rebase fix-grammar-testing`.
...
Rebase on latest fix-grammar-testing and push:

```bash
git fetch origin fix-grammar-testing && git rebase origin/fix-grammar
git push -u origin HEAD
```

Provide the PR creation link targeting `fix-grammar-testing` (as clickable link, NOT in a code block):
