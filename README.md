...
This repository contains the specifications related to the Ethereum execution client, specifically the [pyspec](/src/ethereum/forks/frontier/fork.py) and specifications for network upgrades which are detailed in each `/src/ethereum/forks/$FORK/__init__.py`. The [JSON-RPC API specification](https://github.com/ethereum/execution-apis) can be found in a separate repository.
