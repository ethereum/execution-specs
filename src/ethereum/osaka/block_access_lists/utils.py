...
def ssz_encode_list(
    items: tuple, encode_item_fn, max_length: int = None
) -> bytes:
    """
    ...
    if max_length is None:
        ...
        if is_fixed_size:
            ...
            for item in items:
                # Write offset
                result.extend(
                    ssz_encode_uint(offset_start + len(data_section), 4)
                )
                # Encode item data
...
def ssz_encode_storage_change(change: StorageChange) -> bytes:
    ...
    """
    return ssz_encode_uint(
        change.tx_index, 2
    ) + ssz_encode_bytes(  # TxIndex as uint16
        change.new_value
    )  # StorageValue as Bytes32
...
def ssz_encode_balance_change(change: BalanceChange) -> bytes:
    ...
    return ssz_encode_uint(
        change.tx_index, 2
    ) + ssz_encode_uint(  # TxIndex as uint16
        change.post_balance, 32
    )  # Balance as uint256
...
def ssz_encode_account_changes(account: AccountChanges) -> bytes:
    ...
    storage_changes_data = ssz_encode_list(
        account.storage_changes, ssz_encode_slot_changes, MAX_SLOTS
    )
    ...
    storage_reads_data = ssz_encode_list(
        account.storage_reads, ssz_encode_slot_read, MAX_SLOTS
    )
    offsets.append(base_offset + len(data_section))
    ...
    balance_changes_data = ssz_encode_list(
        account.balance_changes, ssz_encode_balance_change, MAX_TRANSACTIONS
    )
    offsets.append(base_offset + len(data_section))
    ...
    nonce_changes_data = ssz_encode_list(
        account.nonce_changes, ssz_encode_nonce_change, MAX_TRANSACTIONS
    )
    offsets.append(base_offset + len(data_section))
    ...
    code_changes_data = ssz_encode_list(
        account.code_changes, ssz_encode_code_change, MAX_TRANSACTIONS
    )
    offsets.append(base_offset + len(data_section))
...
def ssz_encode_block_access_list(bal: BlockAccessList) -> Bytes:
    ...
    encoded = ssz_encode_list(
        bal.account_changes, ssz_encode_account_changes, MAX_ACCOUNTS
    )
