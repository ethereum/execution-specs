...
class Header:
    ...
    [cbalh]: ref:ethereum.forks.bogota.block_access_lists.hash_block_access_list
    """  # noqa: E501
...
class FrameReceipt:
    """
    Result of executing a single frame of a frame transaction, as
    defined in [EIP-8141].
    ...
    logs: Tuple[Log, ...]
    """
    A tuple of logs generated by the frame.
    """
...
class FrameTransactionReceipt:
    """
    Result of executing a frame transaction, as defined in [EIP-8141].
    Contains one [`FrameReceipt`] per frame instead of a single
    transaction-level status.

    [`FrameReceipt`]: ref:ethereum.forks.bogota.blocks.FrameReceipt
    [EIP-8141]: https://eips.ethereum.org/EIPS/eip-8141
    """
    ...
    """
    The per-frame execution results, in frame order.
    """
...
def encode_receipt(
    ...
    - FrameTransaction receipts are prefixed with `b"\x06"` and use the
      frame receipt payload defined in [EIP-8141].
    - LegacyTransaction receipts are returned as is.

    [EIP-8141]: https://eips.ethereum.org/EIPS/eip-8141
    """
    if isinstance(tx, FrameTransaction):
        assert isinstance(receipt, FrameTransactionReceipt)
