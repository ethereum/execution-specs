...
def create(evm: Evm) -> None:
    ...
    charge_gas(evm, G.G_CREATE + extend_memory.cost + init_code_gas)
...
def create2(evm: Evm) -> None:
    ...
    charge_gas(
        evm,
        G.G_CREATE
        + G.G_KECCAK_256_WORD * call_data_words
        + extend_memory.cost
        + init_code_gas,
    )
...
def return_(evm: Evm) -> None:
    ...
    charge_gas(evm, G.G_BASE + extend_memory.cost)
...
def call(evm: Evm) -> None:
    ...
    if to in evm.accessed_addresses:
        access_gas_cost = G.G_WARM_ACCOUNT_ACCESS
    else:
    ...
    create_gas_cost = G.G_NEW_ACCOUNT
    if value == 0 or is_account_alive(evm.message.block_env.state, to):
        create_gas_cost = Uint(0)
    transfer_gas_cost = Uint(0) if value == 0 else G.G_CALL_VALUE
...
def selfdestruct(evm: Evm) -> None:
    ...
    # GAS
    gas_cost = G.G_SELF_DESTRUCT
    if beneficiary not in evm.accessed_addresses:
        evm.accessed_addresses.add(beneficiary)
        gas_cost += G.G_COLD_ACCOUNT_ACCESS
