"""
The [Engine API] under the Prague fork.

The consensus layer drives the execution layer through versioned
methods — `engine_newPayloadV1`…`V4`, `engine_forkchoiceUpdatedV1`…`V3`, `engine_getPayloadV1`…`V4` — each a thin
...
superseded by Prague answer with an unsupported-fork error, so this
module is the complete engine surface a Prague client presents.
