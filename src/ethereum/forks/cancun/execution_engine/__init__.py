"""
The [Engine API] under the Cancun fork.

The consensus layer drives the execution layer through versioned
methods — `engine_newPayloadV1`…`V3`, `engine_forkchoiceUpdatedV1`…`V3`, `engine_getPayloadV1`…`V3` — each a thin
...
superseded by Cancun answer with an unsupported-fork error, so this
module is the complete engine surface a Cancun client presents.
