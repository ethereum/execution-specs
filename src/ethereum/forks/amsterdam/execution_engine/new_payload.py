...
def new_payload_v1(
    ...
    """
    `engine_newPayloadV1` serves forks before Amsterdam; once Amsterdam is
...
def new_payload_v2(
    ...
    """
    `engine_newPayloadV2` serves forks before Amsterdam; once Amsterdam is
    active it must not be used.
    """
    raise UnsupportedForkError(
...
def new_payload_v3(
    ...
    """
    `engine_newPayloadV3` serves forks before Amsterdam; once Amsterdam is
    active it must not be used.
    """
    raise UnsupportedForkError(
...
def new_payload_v4(
    ...
    _versioned_hashes: Tuple[Hash32, ...],
    _parent_beacon_block_root: Root,
    _execution_requests: Tuple[Bytes, ...],
) -> PayloadStatusV1:
    """
    `engine_newPayloadV4` serves forks before Amsterdam; once Amsterdam is
    active it must not be used.
    """
    raise UnsupportedForkError(
        "engine_newPayloadV4 does not serve the active fork"
    )
