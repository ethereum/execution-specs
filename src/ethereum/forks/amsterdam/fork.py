...
# EIP-8304 has not assigned an address to the index contract yet.
INDEX_CONTRACT_ADDRESS = hex_to_address(
    "0x0000000000000000000000000000000000008304"
)
...
class ChainContext:
    ...
    index_tables: Dict[Uint, IndexTable]
    """Index tables of recent blocks, keyed by block number."""
...
class BlockChain:
    ...
    state: State
    chain_id: U64
    index_tables: Dict[Uint, IndexTable] = field(default_factory=dict)
    """Index tables of recent blocks, keyed by block number."""
...
def state_transition(chain: BlockChain, block: Block) -> None:
    ...
    chain_context = ChainContext(
        ...
        index_tables=chain.index_tables,
    )
    ...
    for number in list(chain.index_tables):
        if number + RETAINED_BLOCKS < block.header.number:
            del chain.index_tables[number]
...
def execute_block(
    ...
    block_env = vm.BlockEnvironment(
        ...
        index_tables=chain_context.index_tables,
    )
...
def update_index_tables(
    ...
    """
    Publish the root of every index table this block completes.

    The entries generated by the block's transactions, together with the hash
    of its parent, make up the block's own table, which is recorded so that
    later blocks can merge it into their own. Tables of the levels above
    cover ranges that ended some blocks ago: publishing them lags the end of
    their range by a quarter of the blocks they cover, leaving room to merge
    them from the level below between blocks rather than during one.

    A range reaching back before index tables were introduced has no table.
    """
    block_env.index_tables[block_env.number] = tuple(
        sorted(
            block_output.index_entries
            + (
                encode_block_entry(
                    block_env.number - Uint(1), block_env.block_hashes[-1]
                ),
            )
        )
    )
    ...
    for table_size in TABLE_SIZES:
        lag = table_size + table_size // Uint(4) - Uint(1)
        if block_env.number < lag:
            continue

        first_block = block_env.number - lag
        if first_block % table_size != Uint(0):
            continue

        table = merge_tables(block_env.index_tables, first_block, table_size)
        if table is None:
            continue

        process_unchecked_system_transaction(
            block_env=block_env,
            target_address=INDEX_CONTRACT_ADDRESS,
            data=first_block.to_be_bytes32()
            + table_size.to_be_bytes32()
            + compute_table_root(table),
        )
...
def apply_body(
    ...
    update_index_tables(block_env, block_output)
...
def process_transaction(
    ...
    assert tx_env.tx_hash is not None
    block_output.index_entries += encode_transaction_entries(
        block_env.number,
        index,
        tx_env.tx_hash,
        ulen(block_output.block_logs),
        tx_output.logs,
    )
