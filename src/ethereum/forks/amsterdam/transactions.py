...
"""
Cap on the execution-gas a single transaction can spend, per [EIP-7825].
Under [EIP-8037] it bounds only the execution-gas dimension, so a
transaction's `gas` may exceed it; the excess feeds the state gas
reservoir.
...
TX_MAX_TOTAL_GAS_LIMIT = Uint(4_294_967_295)
"""
Cap on a transaction's `gas` as a whole, per [EIP-8037].

Unlike [`TX_MAX_GAS_LIMIT`], which applies to execution-gas only, this
bound applies to the sum of both gas dimensions, so that every
transaction's gas limit fits in an unsigned 32-bit integer.
...
def validate_transaction(tx: Transaction, sender: Address) -> IntrinsicGasCost:
    ...
    [`TX_MAX_GAS_LIMIT`]: ref:ethereum.forks.amsterdam.transactions.TX_MAX_GAS_LIMIT
    [`TX_MAX_TOTAL_GAS_LIMIT`]: ref:ethereum.forks.amsterdam.transactions.TX_MAX_TOTAL_GAS_LIMIT
    ...
    if tx.gas > TX_MAX_TOTAL_GAS_LIMIT:
