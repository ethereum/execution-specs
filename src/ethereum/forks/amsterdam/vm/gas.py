...
class StateGasReservoir:
    ...
    `gas_left` first, in LIFO order, on a refund or failure, and repaid
    from the reservoir when a successful child merges
...
def repay_state_gas_spill(gas_meter: GasMeter) -> None:
    ...
    Reservoir-model only: the meter must carry a reservoir. Frame
    transactions have no spill to repay, their state gas living on the
    frame context.
