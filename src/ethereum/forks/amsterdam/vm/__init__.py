...
def incorporate_child(evm: Evm, child_evm: Evm) -> None:
    ...
    In the reservoir model, a successful merge ends with the reservoir
    repaying any [spill] still outstanding: a cross-frame refund lands
    in the reservoir while the `gas_left` that funded the charge stays
    reduced, and the merge is where the claim and the credit first
    share a meter. A failed child repays nothing -- its rollback
    restored the state whose removal any refund credited.
