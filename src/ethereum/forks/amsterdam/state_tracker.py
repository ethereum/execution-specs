...
def get_block_access_index(root_frame: StateChanges) -> BlockAccessIndex:
    """
    Get the current block access index from the root frame.
...
def track_storage_write(
    ...
    """
    state_changes.storage_writes[(address, key, block_access_index)] = value
...
def track_balance_change(
    ...
    """
    state_changes.balance_changes[(address, block_access_index)] = new_balance
...
def track_nonce_change(
    ...
    """
    state_changes.nonce_changes.add((address, block_access_index, new_nonce))
...
def track_code_change(
    ...
    """
    state_changes.code_changes[(address, block_access_index)] = new_code
...
def merge_on_success(child_frame: StateChanges) -> None:
    ...
    Filters net-zero changes by comparing against tx frame's pre-state.
    ...
    # Get the transaction frame for pre-state lookups
    tx_frame = get_transaction_frame(child_frame)
    ...
    for (addr, key, idx), value in child_frame.storage_writes.items():
        ...
        if (addr, key) in tx_frame.pre_storage:
            if tx_frame.pre_storage[(addr, key)] != value:
                ...
                # Net-zero write - convert to read and remove any stale
                # parent write (child's value supersedes parent's)
                parent_frame.storage_reads.add((addr, key))
                if (addr, key, idx) in parent_frame.storage_writes:
                    del parent_frame.storage_writes[(addr, key, idx)]
        else:
    ...
    for (addr, idx), final_balance in child_frame.balance_changes.items():
        if addr in tx_frame.pre_balances:
            if tx_frame.pre_balances[addr] != final_balance:
...
def normalize_balance_changes_for_transaction(
    ...
    """
    Remove net-zero balance changes before committing to block frame.

    Compares pre vs post balance for each address; removes if equal.
    ...
    addresses_to_check = [
        addr
        for (addr, idx) in tx_frame.balance_changes.keys()
