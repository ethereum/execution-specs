...
def account_existed_at_tx_entry(
    ...
    """
    Return whether an account existed at the start of the current
    transaction (i.e., before any tx-level writes).
    """
    if address in tx_state.created_accounts:
        return False
    if address in tx_state.parent.account_writes:
        return tx_state.parent.account_writes[address] is not None
    return tx_state.parent.pre_state.get_account_optional(address) is not None


def compute_state_byte_diff(
    snapshot: TransactionState, tx_state: TransactionState
) -> int:
    """
    ...
    value (via ``get_storage_original``):
      ...
      * Cleared slot, zero at tx start (exit zero, frame-entry
        non-zero, tx-entry zero): -32
      * Cleared slot, non-zero at tx start: 0 (the regular-gas
        ``refund_counter`` path remains inline at SSTORE per
        EIP-3529)
      * Other transitions: 0
    ...
    for address, slots in tx_state.storage_writes.items():
        for key, value_now in slots.items():
            frame_entry = storage_at_snapshot(snapshot, address, key)
            tx_entry = get_storage_original(tx_state, address, key)
            if (
                value_now != U256(0)
                and frame_entry == U256(0)
                and tx_entry == U256(0)
            ):
                delta += 32
            elif (
    ...
    for address, slots in snapshot.storage_writes.items():
        if address in tx_state.storage_writes:
            continue
        for key, frame_entry in slots.items():
            if frame_entry == U256(0):
                continue
            tx_entry = get_storage_original(tx_state, address, key)
            if tx_entry == U256(0):
                delta -= 32
