"""
State Types and Pre-State Interface.
...
Define shared types and the ``PreState`` protocol used by the state
transition function.  The ``PreState`` protocol specifies the operations
that any pre-execution state provider must support, allowing multiple
backing implementations (in-memory dict, on-disk database, witness,
etc.).
"""
