...
collection of accounts (The Dao and all it's children) to a recovery contract.

The recovery contract was previous created using normal contract deployment.
"""

from ethereum.dao_fork.state import State, get_account, move_ether
from ethereum.dao_fork.utils.hexadecimal import hex_to_address
...
def do_dao_fork(state: State) -> None:
    """
    Perform an irregular state change, moving all money in the dao accounts
    into the Dao recovery contract.
    ...
    state : `State`
        The state to change.
    """
    for address_str in dao_accounts:
        address = hex_to_address(address_str)
